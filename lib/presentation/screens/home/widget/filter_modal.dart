...
import 'package:source_base/presentation/screens/shared/widgets/date_picker_btn.dart';
...
class _FilterModalState extends State<FilterModal> {
  ...
  final ValueNotifier<String> _dateString =
      ValueNotifier<String>('all_time'.tr());
  final ValueNotifier<List<MemberModel>> _assigneesNotifier =
      ValueNotifier<List<MemberModel>>([]);
  final ValueNotifier<bool> _resetNotifier = ValueNotifier<bool>(false);
  DateTimeRange? _selectedDateRange;
  bool _isLoadingPagingModels = true;
  ...
  final List<Rating> _ratings = [
    Rating(id: '0', name: 'no_rating'.tr()),
    Rating(id: '1', name: '1 star'.tr()),
    Rating(id: '2', name: '2 star'.tr()),
    Rating(id: '3', name: '3 star'.tr()),
    Rating(id: '4', name: '4 star'.tr()),
  ...
  void initState() {
    ...
    if (widget.initialValue != null) {
      ...
      _selectedPagingModels = List.from(widget.initialValue!.tags);
  ...
  Future<void> _loadPagingModels() async {
    try {
      // final response = await _workspaceRepository.getPagingModelList(
      ...
      //         .map((tag) => PagingModel(name: tag.toString()))
      //         .toList();
      //     _isLoadingPagingModels = false;
      //   });
      // }
    } catch (e) {
      if (mounted) {
        setState(() => _isLoadingPagingModels = false);
  ...
  Future<void> _loadSources() async {
    try {
      ...
      //         .map((source) => UtmSourceModel(name: source.toString()))
      //         .toList();
  ...
  void _resetFilters() {
    setState(() {
      ...
      _selectedCategories = [];
      _selectedSources = [];
      _selectedPagingModels = [];
      _selectedRatings = [];
      _selectedDateRange = null;
      _dateString.value = 'all_time'.tr();
    ...
    return BlocBuilder<FilterItemBloc, FilterItemState>(
        ...
        builder: (context, state) {
          if (state.status == FilterItemStatus.loading) {
            return _buildLoadingSection('label'.tr());
          }
          return ValueListenableBuilder<bool>(
            ...
            builder: (context, _, __) {
              return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('label'.tr(), style: _titleStyle),
                  const SizedBox(height: 8),
                  ChipsInput<PagingModel>(
                    ...
                    suggestionBuilder: (context, state, tag) =>
                        _buildSuggestion(context, state, tag,
                            _selectedPagingModels.contains(tag)),
                  ),
                  const SizedBox(height: 16),
                ],
              );
    ...
    return ValueListenableBuilder<bool>(
      ...
      builder: (context, _, __) {
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('rating'.tr(), style: _titleStyle),
            const SizedBox(height: 8),
            ChipsInput<Rating>(
              key: ValueKey(_resetNotifier.value),
    ...
    return ValueListenableBuilder<bool>(
      ...
      builder: (context, _, __) {
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('by_type'.tr(), style: _titleStyle),
            const SizedBox(height: 8),
            ChipsInput<Category>(
              key: ValueKey(_resetNotifier.value),
  ...
  Widget _buildSourceSection() {
    ...
    return BlocBuilder<FilterItemBloc, FilterItemState>(
        ...
        builder: (context, state) {
          if (state.status == FilterItemStatus.loading) {
            return _buildLoadingSection('by_source'.tr());
          }
          return ValueListenableBuilder<bool>(
            valueListenable: _resetNotifier,
            builder: (context, _, __) {
              return Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('by_source'.tr(), style: _titleStyle),
                  const SizedBox(height: 8),
                  ChipsInput<UtmSourceModel>(
                    ...
                    chipBuilder: _buildChip,
                    suggestionBuilder: (context, state, source) =>
                        _buildSuggestion(context, state, source,
  ...
  Widget _buildDateRangeSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('by_time'.tr(), style: _titleStyle),
        const SizedBox(height: 8),
        SizedBox(
          height: 40,
          child: DatePickerBtn(
              ...
              child: Scaffold(
                appBar: AppBar(
                  ...
                  bottom: PreferredSize(
                    // Tổng chiều cao = 2px cho line + mặc định 48px cho TabBar
                    preferredSize: const Size.fromHeight(1 + 48),
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        // Thanh ngang 2px
                        Container(
                          height: 1,
                          color:
                              Colors.grey.shade300, // bạn có thể đổi màu tuỳ ý
                        ),
                        // TabBar gốc
                        TabBar(
                ...
                body: Column(
                  children: [
                    Expanded(
                      child: TabBarView(
                        children: [
                          BlocListener<FilterItemBloc, FilterItemState>(
                            listener: (context, state) {
                              if (state.status == FilterItemStatus.success) {
                                setState(() {
                                  // _tags = state.paginges;
                                  _isLoadingPagingModels = false;
                                });
                              }
