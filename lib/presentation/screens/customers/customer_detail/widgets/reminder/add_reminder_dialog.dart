...
class _AddReminderDialogState extends State<AddReminderDialog> {
  ...
  List<Map<String, int>> _notifyBeforeList =
  ...
  void _initializeForm() {
    if (widget.editingReminder != null) {
      ...
      _selectedPriority = Priority.fromValue(reminder.priority ?? 0);
      _isDone = reminder.isDone ?? false;
      ...
      try {
        ...
        _startDate = DateTime(startDateTimeLocal.year, startDateTimeLocal.month,
            startDateTimeLocal.day);
        ...
        if (reminder.endTime != null) {
          ...
          _endDate = DateTime(endDateTimeLocal.year, endDateTimeLocal.month,
              endDateTimeLocal.day);
      ...
      if (reminder.contact != null) {
        _selectedContact = {
          ...
          'avatar': reminder.contact ?? '',
          'phone': reminder.contact ?? '',
        };
      }

      // Initialize notification list from existing reminder
      if (reminder.reminders != null && reminder.reminders!.isNotEmpty) {
  ...
  Future<void> _saveReminder() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(widget.editingReminder != null
              ? 'Đã cập nhật nhắc hẹn'
              : 'Đã tạo nhắc hẹn'),
          backgroundColor: Colors.green,
        ),
      );
