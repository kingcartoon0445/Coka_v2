...
class _NotificationListWidgetState extends State<NotificationListWidget> {
  ...
  void initState() {
    super.initState();
    _notificationRepository = NotificationRepository(DioClient());
  ...
  void _navigateBasedOnNotificationType(
    ...
    final String organizationId =
        notification['organizationId'] ?? widget.organizationId ?? '';
    ...
    if (organizationId != currentOrgId && organizationId.isNotEmpty) {
      ...
      try {
        await SharedPreferencesService()
            .setString(PrefKey.defaultOrganizationId, organizationId);
    ...
    } else if (['VERIFY_WEBSITE', 'CONNECT_FORM', 'EXPIRED_ACCESSTOKEN']
        .contains(category)) {
    ...
    if (route.isNotEmpty) {
      ...
      if (organizationId != currentOrgId) {
        ...
        if (isRouteWithoutShell) {
          print(
              'Navigate đến tổ chức khác qua route ngoài shell, đi qua base trước: $route');
        ...
        if (widget.fullScreen) {
          ...
          if (isRouteWithoutShell) {
            ...
            print(
                'Compact view - sử dụng context.go cho workspace route: $route');
            context.go(route);
          } else {
            // Routes đơn giản - có thể push an toàn
            print(
                'Compact view - sử dụng context.push cho route đơn giản: $route');
            context.push(route);
          }
        }
      }
                ...
                children: [
                  RichText(
                    ...
                    text: TextSpan(
                      ...
                      children:
                          _parseHtmlText(notification['contentHtml'] ?? ''),
                    ),
                  ),
  ...
  Widget _buildCompactView() {
    ...
    final hasMore =
        _notifications!.length > (widget.maxItems ?? _notifications!.length);
