...
class AssignToBottomSheet extends StatefulWidget {
  ...
  final List<Assignees>? defaultAssignees;
...
class _AssignToBottomSheetState extends State<AssignToBottomSheet>
  ...
  final TextEditingController _orgMemberSearchController =
      TextEditingController();
  final TextEditingController _salesMemberSearchController =
      TextEditingController();
  ...
  List<Assignees> _orgMembers = [];
  List<Assignees> _salesMembers = [];
  List<Assignees> _teams = [];
  final List<Assignees> _selectedMembers = [];
    ...
    try {
      ...
      if (mounted && !_orgMembersCancelToken!.isCancelled) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content:
                  Text('Có lỗi xảy ra khi tải danh sách thành viên tổ chức')),
        );
      }
    } finally {
      if (mounted && !_orgMembersCancelToken!.isCancelled) {
    ...
    try {
      ...
      if (mounted && !_salesMembersCancelToken!.isCancelled) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content:
                  Text('Có lỗi xảy ra khi tải danh sách thành viên đội sale')),
        );
      }
    } finally {
      if (mounted && !_salesMembersCancelToken!.isCancelled) {
  ...
  Widget _buildCompactSearchBar() {
    ...
    final controller =
        isOrgTab ? _orgMemberSearchController : _salesMemberSearchController;
    final hintText =
        isOrgTab ? 'Tìm thành viên tổ chức...' : 'Tìm thành viên đội sale...';
  ...
  void _handleOrgMemberSelect(Assignees member) {
    setState(() {
      final index = _selectedMembers.indexWhere(
        (selected) {
          ...
          return selected.profileName == member.profileName;
        },
      );
  ...
  void _handleSalesMemberSelect(Assignees member) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (dialogContext) => CustomAlertDialog(
        title: 'Chuyển phụ trách?',
        subtitle: 'Bạn có chắc muốn phân phối data đến ${member.profileName}?',
        onSubmit: () async {
          ...
          try {
            ...
            if (mounted && !_assignCancelToken!.isCancelled) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                    content: Text(
                        'Đã chuyển phụ trách thành công cho ${member.profileName}')),
              );
            }
          } catch (e) {
            if (mounted && !_assignCancelToken!.isCancelled) {
  ...
  void _handleTeamSelect(Assignees team) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (dialogContext) => CustomAlertDialog(
        title: 'Chuyển phụ trách?',
        subtitle:
            'Bạn có chắc muốn phân phối data đến đội sale ${team.profileName}?',
        onSubmit: () async {
          Navigator.pop(dialogContext);
          ...
          try {
            ...
            if (mounted && !_assignCancelToken!.isCancelled) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                    content: Text(
                        'Đã chuyển phụ trách thành công cho đội sale ${team.profileName}')),
              );
            }
          } catch (e) {
            if (mounted && !_assignCancelToken!.isCancelled) {
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isSelected = _selectedMembers.any(
                          (selected) {
                            ...
                            final selectedId =
                                selected.profileId ?? selected.id;
                            final memberId = member.profileId ?? member.id;
                            ...
                            child: Row(
                              children: [
                                AppAvatar(
                                  ...
                                  fallbackText: member.profileName,
                                  size: 44,
                                ),
